...
public class FrmDadosGrupo extends javax.swing.JDialog {
    ...
    public final void readPermissao() {
        ...
        for (Form form : formDAO.findEntities(Form.class)) {
    ...
    public final void readPermissaoGrupo() {
        ...
        categoryMapAcesso.clear();
        AcessoDAO acessDAO = new AcessoDAO();
        RegraAcessoDAO regraAcessDAO = new RegraAcessoDAO();
        for (Acesso acess : acessDAO.findEntities(Acesso.class)) {
            Integer id = acess.getId();
            String name = acess.getAcesso();
            comboAcesso.addItem(name);
        ...
        TableColumn colAcesso = tblPermissao.getColumnModel().getColumn(2);
        colAcesso.setCellEditor(new DefaultCellEditor(comboAcesso));
        for (RegraAcesso regraAcess : regraAcessDAO.getListAcesso(grupo.getId())) {
            model.addRow(new Object[]{
                regraAcess.getForm().getId(),
                regraAcess.getForm().getFormName(),
                regraAcess.getAcesso().getAcesso(),
                regraAcess.getId()
            });
        }
    }
    ...
    private void criarPermissao(Integer acesso_id, Integer form_id, Integer grupo_id) {
        try {
            ...
            Form form = new Form();
            acesso.setId(acesso_id);
            form.setId(form_id);
            grup.setId(grupo_id);
            regraAcesso.setForm(form);
            regraAcesso.setGrupo(grup);
